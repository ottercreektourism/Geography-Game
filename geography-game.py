...
def generate_random_country_index(visited_countries_indices):
    ...
    finds one that doesn't exist in the list of visited countries indices."""
...
def user_turn(answer, user_score, player_number):
    """ Function that increases the user_score if they got the right answer."""
...
def answer_contains_expected_chars(user_answer, user_answer_type):
    """ Function that checks if the user answer doesn't contain unexpected characters. """
...
def the_answer_is(answer):
    """ Function that displays the answer, whatever its type is, appropriately."""
...
def ask_question(answer):
    """Function that asks a question depending on the answer given. """
    print("")
    print("What", end = " ")
